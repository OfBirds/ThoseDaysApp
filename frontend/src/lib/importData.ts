...
export function parseImport(text: string): { doc: ImportDoc } | { error: string } {
  ...
  try {
    ...
    return { error: "That doesn't look like a Pink Cockatoo file — it isn't valid JSON." };
  }
  const obj = raw as Partial<ImportDoc>;
  if (typeof obj.schemaVersion !== 'number')
    return { error: "This file is missing its version — it may not be a Pink Cockatoo export." };
